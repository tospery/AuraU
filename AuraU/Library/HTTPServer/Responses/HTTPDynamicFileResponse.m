...
- (void)processReadBuffer
{
	...
	{
		const void *subBuffer = (const void *)((NSUInteger)readBuffer + offset);
		...
		if (memcmp(subBuffer, sep, sepLen) == 0)
		{
			...
			if (found1 && found2)
			{
				...
				void *strBuf = (void *)((NSUInteger)readBuffer + strRange.location);
				...
				if (key)
				{
					...
					if (value)
					{
						...
						if (fullRange.length == vLength)
						{
							...
							memcpy((void *)((NSInteger)readBuffer + fullRange.location), [v bytes], vLength);
						}
						else // (fullRange.length != vLength)
						{
							...
							void *src = (void *)((NSInteger)readBuffer + fullRange.location + fullRange.length);
							void *dst = (void *)((NSInteger)readBuffer + fullRange.location + vLength);
	...
	else
	{
		...
		memmove(readBuffer, (void *)((NSInteger)readBuffer + available), remaining);
