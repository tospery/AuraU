...
#define kMJBgclickEnabled           @"kMJBgclickEnabled"
...
- (BOOL)bgclickEnabled {
    return [objc_getAssociatedObject(self, kMJBgclickEnabled) boolValue];
}

- (void)setBgclickEnabled:(BOOL)bgclickEnabled {
    objc_setAssociatedObject(self, kMJBgclickEnabled, [NSNumber numberWithBool:bgclickEnabled], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)presentPopupViewController:(UIViewController*)popupViewController animationType:(MJPopupViewAnimation)animationType bgclickEnabled:(BOOL)bgclickEnabled dismissed:(void(^)(void))dismissed
{
    if (self.mj_popupViewController) {
        [self dismissPopupViewControllerWithanimationType:animationType animated:NO];
    }
    popupViewController.bgclickEnabled = bgclickEnabled;
...
{
    [self presentPopupViewController:popupViewController animationType:animationType bgclickEnabled:bgclickEnabled dismissed:nil];
}

- (void)dismissPopupViewControllerWithanimationType:(MJPopupViewAnimation)animationType {
    [self dismissPopupViewControllerWithanimationType:animationType animated:YES];
}
...
- (void)dismissPopupViewControllerWithanimationType:(MJPopupViewAnimation)animationType animated:(BOOL)animated
{
    ...
    switch (animationType) {
        ...
        case MJPopupViewAnimationSlideRightRight:
            [self slideViewOut:popupView sourceView:sourceView overlayView:overlayView withAnimationType:animationType animated:animated];
            break;
            
        default:
            [self fadeViewOut:popupView sourceView:sourceView overlayView:overlayView animated:animated];
            break;
    }
}
...
{
//    UIViewController *aa = self.mj_popupViewController;
//    BOOL bb = aa.disableBackgroundDismiss;

    if (!self.mj_popupViewController.bgclickEnabled) {
        return;
    }
...
- (void)slideViewOut:(UIView*)popupView sourceView:(UIView*)sourceView overlayView:(UIView*)overlayView withAnimationType:(MJPopupViewAnimation)animationType animated:(BOOL)animated
{
    // Generating Start and Stop Positions
    CGSize sourceSize = sourceView.bounds.size;
    CGSize popupSize = popupView.bounds.size;
