...
+(GDataXMLNode*) createCData:(NSString*)content
{
    const xmlChar *text = GDataGetXMLString(content);
    xmlNodePtr theNewNode=xmlNewCDataBlock(NULL,
                                           text,
                                           strlen(text));
    if (theNewNode) {
        // succeeded
        return [GDataXMLNode nodeConsumingXMLNode:theNewNode];
    }
    return nil;
}
